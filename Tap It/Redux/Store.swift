...
extension Redux {
    static let store = Store(state: .init(tappedProfiles: State.TappedProfilesState(profiles: [:])), reducer: Reducer.main, middleware: middleware, publishOn: DispatchQueue.main)
}
